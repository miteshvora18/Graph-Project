...
{
    public partial class SelectAns : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if (txtQuestion.Text != "")
                {
                    if (s.Length == cnt - 1)
                    {
                        ...
                        con = new SqlConnection(conn);
                        SqlCommand cmd = new SqlCommand("Update questions set qname=@qname,answer=@answer,qvalid=@qvalid where qid=@qid", con);
                        cmd.Parameters.AddWithValue("@qname", txtQuestion.Text);
                        cmd.Parameters.AddWithValue("@answer", singleItem);
                        cmd.Parameters.AddWithValue("@qvalid", true);
                        cmd.Parameters.AddWithValue("@qid", qid);
                        con.Open();
                        cmd.ExecuteNonQuery();
                        con.Close();
                        Clear();
                        Response.Redirect("AdminHome.aspx?msg=added");
                    }
                    else
                    {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alert", "alert('Question cannot be empty.')", true);
                }
            }
            catch (Exception e1)
            {
